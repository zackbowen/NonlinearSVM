...
ensemble = False # If true, enable ensemble learning
all_features = False # If true, train and test on all features
...
def main():
    ...
    m = 2
    c = 1
    gamma = 1 / len(x_test[0,:]) # 1 / number of features
    ...
    if all_features:
        ...
        # Holds predictions across all SVMs
        y_pred = np.zeros((len(x),len(SVMs))) # Samples x SVMs
        Z = np.zeros((len(np.c_[xx.ravel(), yy.ravel()]),len(SVMs)))
        for i in range(len(SVMs)):
            # Predict
            svm = SVMs[i]
            y_pred[:,i] = np.transpose(svm.testSVM(x))

            # Put the result into a color plot
            Z[:,i] = np.transpose(svm.testSVM(np.c_[xx.ravel(), yy.ravel()]))

        # Voting system
        Z = np.sign(np.sum(Z,axis=1))
        y_pred = np.sign(np.sum(y_pred,axis=1))
        print(str.format("{0} Accuracy: {1:.3f}", "Ensemble", accuracy_score(y, y_pred)))
        ...
        plt.savefig(str.format("./graphs/{0}_{1}_ensemble.png",features[c1],features[c2]),bbox_inches='tight')
        plt.clf()
    else:
